...
// ==========================================================
// ⚙️ إعداد المتغيرات المباشرة داخل الكود (تحديث المفاتيح)
// ==========================================================
const PORT = process.env.PORT || 3000;
const SUPABASE_URL = "https://qfcsaiyuyxhibidrrmha.supabase.co"; // رابط قاعدة البيانات الخاص بك
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImV2Y3dyaHNneXJkeHB6ZG5lenl3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI4MzQxMzYsImV4cCI6MjA5ODQxMDEzNn0.xjeQCgQLm4xxlwmviIUUe_efRFzCA1hrb3Cu4ld0Ncw"; // رمز السوبابيز الخاص بك
const FIRECRAWL_API_KEY = "fc-a700fd57df374eac9b1f0ce215aab2e5"; // رمز الـ Firecrawl الخاص بك

// إعداد كاش محلي في الذاكرة (بديل لكاش Cloudflare)
const rateLimitCache = new Map();
const mainDataCache = new Map();

// الـ Headers الخاصة بالـ CORS
const corsHeaders = {
...
// المسار الرئيسي للبحث (يدعم GET و POST)
app.all('/', async (req, res) => {
  res.set(corsHeaders);

  try {
    // --- 1. نظام الحماية يعتمد على الـ IP ---
    const userIP = req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'anonymous';
    const currentTime = Math.floor(Date.now() / 1000);

    const lastRequestTime = rateLimitCache.get(userIP) || 0;
    ...
    rateLimitCache.set(userIP, currentTime);
    setTimeout(() => rateLimitCache.delete(userIP), 5000);
    ...
    // ==========================================================
    // 🛡️ [المستوى 1] الكاش المحلي بالسيرفر
    // ==========================================================
    const cachedData = mainDataCache.get(databasePhone);
    if (cachedData && (Date.now() - cachedData.timestamp < 259200 * 1000)) { // 3 أيام صلاحية
      res.set('X-Cache-Status', 'HIT');
      res.set('X-Cache-Level', 'RAILWAY_MEMORY_CACHE');
      ...
      try {
        ...
        if (dbResponse.ok) {
          ...
          if (existingRecords && existingRecords.length > 0) {
            ...
            mainDataCache.set(databasePhone, { data: finalResponseData, timestamp: Date.now() });
    ...
    // الـ Fallback إلى Firecrawl إذا فشل الكشط المباشر
    ...
    if (!success || names.length === 0) {
      return res.status(200).json({ 
        ...
        debug: { phone: scrapePhone, provider: provider }
      });
    }
...
function extractNamesFromResponse(html) {
  ...
  while ((listMatch = listPattern.exec(html)) !== null) {
    let name = listMatch[1] || listMatch[2] || '';
    name = cleanExtractedName(name);
